...
def create_env(env_type, grid_size):

    # Fitness landscapes
    if env_type.lower() == "ackley":
        fitness_m = ackley(grid_size); env_title = "Ackley"
    elif env_type.lower() == "masonwatts":
        fitness_m = mason_watts(grid_size); env_title = "MasonWatts"
    elif env_type.lower() == "crossintray":
        fitness_m = cross_in_tray(grid_size); env_title = "CrossInTray"
    elif env_type.lower() == "shubert":
        fitness_m = shubert(grid_size); env_title = "Shubert"
    elif env_type.lower() == "dropwave":
        fitness_m = drop_wave(grid_size); env_title = "DropWave"
    elif env_type.lower() == "schaffer":
        fitness_m = schaffer(grid_size); env_title = "SchafferN4"
...
def ackley(grid_size=1000, a=20, b=0.2, c=2 * np.pi):
    """
...
def mason_watts(grid_size=1000):
    ...
    # fitness_m = fitness_m * (10 / fitness_m.max())
    # fitness_m = fitness_m * (100 / fitness_m.max())
    fitness_m = fitness_m * (1 / fitness_m.max())

    return fitness_m


def cross_in_tray(grid_size=1000):
...
def holder_table(grid_size=1000):
    x = y = np.linspace(-10, 10, grid_size)
    xx, yy = np.meshgrid(x, y)
