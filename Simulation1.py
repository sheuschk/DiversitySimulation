...
def local_search_2D(env: np.array, agent_value, eta: float, position: np.array, steps, tau, rng):
    """Stochastic Hill climbing OR random step (only if better)"""
    if eta >= rng.random():
        rand_pos = rng.integers(0, env.grid_size, 2)
        rand_value = env.get_fitness(rand_pos)
        if rand_value > agent_value:
            return rand_pos.reshape(1, 2), np.array([1.])
    neighbour_states = [[clip_steps(position[0]+s[0], env.grid_size), clip_steps(position[1]+s[1], env.grid_size)] for s in steps]
...
def local_search(env_type, env, S, eta, solution, all_steps, tau, rng):
    if env_type == "NK":
        new_solutions, probability_distribution = local_search_NK(env, solution, eta)
    elif env_type == "2D":
        new_solutions, probability_distribution = local_search_2D(env, S, eta, solution, all_steps, tau, rng)
...
def run_simulation(args, F, env, rng):
    ...
    S_history[0] = S
    R = S
    W_history = np.empty((args["max_iter"], F.shape[1]))
    W_history[0] = W.mean(axis=0)

    # Repeated processes
    for t in range(1, args["max_iter"]):
        Y = softmax((A/A.sum(axis=1)) / args["tau"], axis=1)
        for agent_i in range(n_agents):
            # (1) & (2) Individual and social optimization
            if gammas[agent_i] >= rng.random():
                probability_distribution = Y[agent_i]
                ...
                new_solutions, probability_distribution = local_search(env_type, env, S[agent_i], etas[agent_i],
                                                             ...
                                                             args["tau"], rng)
            next_solution = draw_step(new_solutions, probability_distribution, rng)
            intermediate_solutions[agent_i] = next_solution
            intermediate_S[agent_i] = env.get_fitness(next_solution)
...
def init_parser():
    ...
    args["n_roleModels"]       = 10  # Number of role models

    all_arg_keys = set(["lr", "tau", "beta", "lambda_scale", "sigma", "intervention_start", "int_duration", "n_roleModels",
...
def save_results(args, file_title):
    dir_name = "Sim1_results"
    if not os.path.exists(dir_name):
        os.mkdir(dir_name)
    inter = f"intervention" if args["intervention"] == True else "no_intervention"
    dir_name += f"/{inter}"
    if not os.path.exists(dir_name):
        os.mkdir(dir_name)
    dir_name += f"/{args['env']}"
    ...
    if args["intervention"] == True:
        file_title += f'_{args["int_duration"]}'

    time = datetime.now().strftime('%Y_%m_%dT%H_%M')
    dir_name += f"/{time}"
    os.mkdir(dir_name)
...
if __name__ == '__main__':
    ...
    if args["env_type"] == "NK":
        ...
        env = Environment2D(args["env"], args["grid_size"])
    file_title = env.get_title()
    sys.stdout.write('\nFinished with reading environment\n')

    # Result saving
    all_results = np.empty((total_simulations*n_agents, 12))  # Second param: How many vars to track
